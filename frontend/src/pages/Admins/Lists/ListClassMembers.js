...
function ListClassMembers({ location: { panelSearchInput } }) {
  ...
  const [update, setUpdate] = useState({ year: selectedYear, value: true });
  ...
  const removeFromClass = useCallback(
    ...
    [class_number, course, unit, selectedYear]
  );
  ...
  useEffect(() => {
    ...
    async function getMembers(username = "") {
      let [students, status] = await adminService.get.studentsFromClass(
        class_number,
        selectedYear,
        course,
        unit
    ...
    if (update.value || update.year !== selectedYear) {
      if (searchInput.initial !== "") {
        getMembers(searchInput.initial);
      } else {
        getMembers();
      }
      setUpdate({ value: false, year: selectedYear });
    }
  }, [
    searchInput,
    course,
