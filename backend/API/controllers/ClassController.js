...
class ClassController {
	constructor() {
		...
		this.findNUClassByYear = this.findNUClassByYear.bind(this);
	...
	async findNUClassByYear(request, response, next) {
		const { year: academic_year } = request.params;
		const classes = await connection('Class')
			.select('number', 'begins_at', 'ends_at', 'week_day', 'academic_year')
			.where({ academic_year });
	...
	async find(request, response, next) {
		...
		const [class_] = await connection('Class')
			.select('number', 'begins_at', 'ends_at', 'week_day', 'academic_year')
			.where({ unit_id: unit.id, number: class_number, academic_year: year });
		if (!class_)
