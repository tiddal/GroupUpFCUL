...
describe('Comment', () => {
	...
	beforeEach(async () => {
		...
		await request(app)
			.post(
				'/courses/L079/units/26719/projects/2019-2020/1/teams/T001/comments/'
			)
			.send({ comment: { message: '1' } })
			.set('Authorization', `Bearer ${studentToken}`);
		await request(app)
			.post(
				'/courses/L079/units/26719/projects/2019-2020/1/teams/T001/comments/'
			)
			.send({ comment: { message: '2' } })
			.set('Authorization', `Bearer ${studentToken}`);
	});
	...
	it('should be able to create a new comment', async () => {
		const response = await request(app)
			...
			.send({ comment: { message: 'nao fiz nada' } })
			.set('Authorization', `Bearer ${studentToken}`);
		expect(response.status).toBe(201);
	});

	it('should be able to get all comments from a team', async () => {
		const response = await request(app)
